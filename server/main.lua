...
CreateThread(function()
    ...
    TriggerClientEvent('kg-garages:client:houseGarageConfig', -1, HouseGarages)
    TriggerClientEvent('kg-houses:client:setHouseConfig', -1, Config.Houses)
end)
...
KGCore.Commands.Add('decorate', Lang:t('info.decorate_interior'), {}, false, function(source)
    local src = source
    TriggerClientEvent('kg-houses:client:decorate', src)
end)

KGCore.Commands.Add('createhouse', Lang:t('info.create_house'), { { name = 'price', help = Lang:t('info.price_of_house') }, { name = 'tier', help = Lang:t('info.tier_number') } }, true, function(source, args)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('kg-houses:client:createHouses', src, price, tier)
    else
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

KGCore.Commands.Add('addgarage', Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('kg-houses:client:addGarage', src)
    else
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

KGCore.Commands.Add('ring', Lang:t('info.ring_doorbell'), {}, false, function(source)
    local src = source
    TriggerClientEvent('kg-houses:client:RequestRing', src)
end)

-- Item

KGCore.Functions.CreateUseableItem('police_stormram', function(source, _)
    local Player = KGCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty) then
        TriggerClientEvent('kg-houses:client:HomeInvasion', source)
    else
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.emergency_services'), 'error')
    end
end)
...
RegisterNetEvent('kg-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent('kg-houses:client:setHouseConfig', src, Config.Houses)
end)

RegisterNetEvent('kg-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent('kg-houses:client:createBlip', -1, coords)
end)

RegisterNetEvent('kg-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", '')
    ...
    TriggerClientEvent('kg-houses:client:setHouseConfig', -1, Config.Houses)
    TriggerClientEvent('KGCore:Notify', src, Lang:t('info.added_house', { value = label }))
    TriggerEvent('kg-log:server:CreateLog', 'house', Lang:t('log.house_created'), 'green', Lang:t('log.house_address', { label = label, price = price, tier = tier, agent = GetPlayerName(src) }))
end)

RegisterNetEvent('kg-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('kg-garages:client:addHouseGarage', -1, house, garageInfo)
    TriggerClientEvent('KGCore:Notify', src, Lang:t('info.added_garage', { value = garageInfo.label }))
end)

RegisterNetEvent('kg-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('kg-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('kg-houses:server:openStash', function(CurrentHouse)
    local src = source
    ...
    if stashSlots and stashWeight then
        exports['kg-inventory']:OpenInventory(src, CurrentHouse, {
...
RegisterNetEvent('kg-houses:server:buyHouse', function(house)
    local src = source
    local pData = KGCore.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.already_owned'), 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('kg-houses:client:SetClosestHouse', src)
        TriggerClientEvent('kg-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, 'bought-house') -- 21% Extra house costs
        exports['kg-banking']:AddMoney('realestate', (HousePrice / 100) * math.random(18, 25), 'House purchase')
        TriggerEvent('kg-log:server:CreateLog', 'house', Lang:t('log.house_purchased'), 'green', Lang:t('log.house_purchased_by', { house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname }))
        TriggerClientEvent('KGCore:Notify', src, Lang:t('success.house_purchased'), 'success', 5000)
    else
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.not_enough_money'), 'error')
    end
end)

RegisterNetEvent('kg-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('kg-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('kg-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('kg-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('kg-houses:server:giveKey', function(house, target)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local pData = KGCore.Functions.GetPlayer(target)
    ...
    if not isHouseOwner(Player.PlayerData.license, Player.PlayerData.citizenid, house) then
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.not_owner'), 'error')
        return
    end
...
RegisterNetEvent('kg-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('KGCore:Notify', src, Lang:t('error.remove_key_from', { firstname = citizenData.firstname, lastname = citizenData.lastname }), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
end)

RegisterNetEvent('kg-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = KGCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('kg-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('kg-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('kg-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('kg-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', { json.encode(decorations), house })
    TriggerClientEvent('kg-houses:server:sethousedecorations', -1, house, decorations)
end)

RegisterNetEvent('kg-houses:server:LogoutLocation', function()
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local tPlayer = KGCore.Functions.GetPlayer(target)
    ...
    if housekeyholders[house] then
        for _, cid in pairs(housekeyholders[house]) do
            if cid == tPlayer.PlayerData.citizenid then
                TriggerClientEvent('KGCore:Notify', src, Lang:t('error.already_keys'), 'error', 3500)
                return
            end
        end
        housekeyholders[house][#housekeyholders[house] + 1] = tPlayer.PlayerData.citizenid
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
        TriggerClientEvent('kg-houses:client:refreshHouse', tPlayer.PlayerData.source)

        TriggerClientEvent('KGCore:Notify', tPlayer.PlayerData.source, Lang:t('success.recieved_key', { value = Config.Houses[house].adress }), 'success', 2500)
    else
        local sourceTarget = KGCore.Functions.GetPlayer(src)
        housekeyholders[house] = {
...
RegisterNetEvent('kg-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('kg-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('kg-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('kg-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('kg-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
KGCore.Functions.CreateCallback('kg-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = KGCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money['bank']
...
KGCore.Functions.CreateCallback('kg-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
KGCore.Functions.CreateCallback('kg-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
KGCore.Functions.CreateCallback('kg-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
KGCore.Functions.CreateCallback('kg-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

KGCore.Functions.CreateCallback('kg-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
KGCore.Functions.CreateCallback('kg-phone:server:TransferCid', function(_, cb, NewCid, house)
...
KGCore.Functions.CreateCallback('kg-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
KGCore.Functions.CreateCallback('kg-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', { house })
    if result[1] then
        retval = result[1]
...
KGCore.Functions.CreateCallback('kg-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = KGCore.Functions.GetPlayer(src)
    if pData then
...
KGCore.Functions.CreateCallback('kg-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = KGCore.Functions.GetPlayer(src)
...
KGCore.Functions.CreateCallback('kg-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local MyHouses = {}
...
KGCore.Functions.CreateCallback('kg-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local MyKeys = {}
...
KGCore.Functions.CreateCallback('kg-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
